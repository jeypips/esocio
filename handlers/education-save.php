...
$profile_sector = array(
	"profile_id"=>1,
	"sector_id"=>$_POST['sectors']['education']['id']
);

$con->insertData($profile_sector);
$profile_sector_id = $con->insertId;

foreach ($_POST['sectors']['education']['parameters'] as $i => $parameter) {
	...
	foreach ($_POST['sectors']['education']['parameters'][$i]['items'] as $ii => $item) {
